{
  "claims": [
    ...
    },
    {
      ...
      "spec_location": "§0.2 Binding constraints",
      "evidence": "[inferred existing-artefact claim; 'are REMOVED' presupposes the artefact exists] Read Domain/MaterialComplexityEditor.fs:140 -> `| UnsupportedComplexity of reason : string` (case decl); :784 -> `| Some (MuWithDispValue _) ->` dispersive-Polder-mu arm whose `Error (UnsupportedComplexity ...)` continues at 785; :790 -> `| Some (RhoWithDispValue _) ->` dispersive-gyration arm whose `Error (UnsupportedComplexity ...)` continues at 791. All three cited lines head the case decl plus the two view-only fallback arms; present.",
      "verdict": "CONFIRMED"
    },
    {
      "id": 5,
      ...
      "spec_location": "§0.2 Binding constraints",
      "evidence": "[inferred existing-artefact claim; 'are REMOVED' presupposes the artefact exists] Read Domain/DispersionModels.fs:505 -> `| NotAFiniteTermSum of reason : string` (case decl); :666 -> `| TaucLorentz _ ->` first transcendental-rejection arm whose `Error (NotAFiniteTermSum ...)` continues at 667 (GaussianOscillator/ForouhiBloomer/BrendelBormann follow at 668-673). Read Domain/MaterialComplexityEditor.fs:132 -> `| SegmentNotLowerable of reason : string` (case decl); :637 -> `| Error (NotAFiniteTermSum reason) -> Error (SegmentNotLowerable $\"segment %d{segmentIndex}: %s{reason}\")` (the lowering-rejection wiring). All four cited sites present.",
      "verdict": "CONFIRMED"
    },
    {
      "id": 6,
      ...
      "evidence": "Read Domain/ElementId.fs:616 -> `static member createInMemory () : SampleProxy =` (closes over a `ref (Map ...)`); :686 -> `static member createInMemory (samplesReferencing : MaterialId -> Sample list) : MaterialProxy =` (closes over a `ref (... Map.ofList)`). Both cited members present at their lines.",
      "verdict": "CONFIRMED"
    },
    {
      "id": 7,
      ...
      "spec_location": "§0.3 Binding constraints",
      "evidence": "Read Domain/ElementId.fs:276 -> `type LibraryProxy =` (a record of `Result`-returning functions). Cited symbol/line present.",
      "verdict": "CONFIRMED"
    },
    {
      "id": 8,
      ...
      "evidence": "[inferred existing-artefact claim; 'following <existing>' precedent phrasing] Class-anchored: Glob resolves TestWindows/MaterialEditorView.fs and TestWindows/SampleEditorView.fs (both files exist). Cited precedent views present.",
      "verdict": "CONFIRMED"
    },
    {
      "id": 9,
      ...
      "evidence": "[inferred existing-wiring claim] Read Ui/OpticalConstructor.Ui.fsproj:176 -> `<PackageReference Include=\"Microsoft.Web.WebView2\" Version=\"1.0.4078.44\" />`. Cited reference present at the line.",
      "verdict": "CONFIRMED"
    },
    {
      "id": 10,
      ...
      "spec_location": "§0.6 Binding constraints",
      "evidence": "The bare `:75,80` inherits the ChartHosts adapter file just named. Read Ui/ChartHosts.fs:72-86 -> the '(b) WebView2 host for a Plotly.NET.GenericChart' section: :75 `/// Attempt to host a Plotly chart by navigating a WebView2 to the chart's embedded HTML`; :79-80 `the \\`Microsoft.Web.WebView2\\` package ships its control assemblies for \\`net462\\` / Windows-desktop TFMs only`. Cited lines are exactly the ChartHosts WebView2 adapter region.",
      "verdict": "CONFIRMED"
    },
    {
      "id": 11,
    ...
    },
    {
      ...
      "spec_location": "§A.0 Problem statement",
      "evidence": "Read Domain/MaterialLibrary.fs:119 -> `category : MaterialCategory` (field of the `MaterialEntry` record; `id : MaterialId` is at 117). Cited field/line present.",
      "verdict": "CONFIRMED"
    },
    {
      "id": 13,
      ...
      "spec_location": "§A.0 Problem statement",
      "evidence": "Read Domain/MaterialLibrary.fs:368 -> `category : MaterialCategory option` (field of the `MaterialQuery` record; default None at 376). Cited field/line present.",
      "verdict": "CONFIRMED"
    },
    {
      "id": 14,
    ...
    },
    {
      ...
      "spec_location": "§A.0 Problem statement",
      "evidence": "Read Domain/MaterialLibrary.fs -> `let byQuery (q : MaterialQuery) (lib : MaterialLibrary) : MaterialEntry list =` is declared at 415; the cited line 417 is `let byCat =`, byQuery's own category-filtering body (417-420 applies `byCategory` to the query's `category` facet, `match q.category with | Some c -> byCategory c ...`). The cited line falls inside the named function and holds exactly the 'filtered by byQuery' content, so the pointer resolves (2 lines from the `let byQuery` header, within the function span 415-424).",
      "verdict": "CONFIRMED"
    },
    {
      "id": 16,
    ...
    },
    {
      ...
      "evidence": "[inferred existing-artefact claim] Read Domain/MaterialLibrary.fs:314 -> `id = MaterialIds.vacuum` inside the vacuum `MaterialEntry` (315 name \"Vacuum\", 316 category Vacuum, 317 description). Cited seeded vacuum material present at the line.",
      "verdict": "CONFIRMED"
    },
    {
      "id": 19,
      ...
      "evidence": "[inferred existing-artefact claim; 'mirroring <existing>' precedent phrasing] Read Domain/MaterialLibrary.fs:135 -> `| MaterialStillReferenced of reason : string` (MaterialError case); Read Domain/ElementId.fs:715 -> `removeMaterial =` whose body (715-729) consults `samplesReferencing` and returns `Error (MaterialStillReferenced ...)` naming the referencing samples. Both cited sites present.",
      "verdict": "CONFIRMED"
    },
    {
      "id": 20,
      ...
      "spec_location": "§A.0 Recorded decisions (Removal never cascades)",
      "evidence": "['as ... today' assumption-shape] Read Domain/ElementId.fs:740 -> `let samplesReferencing (samples : SampleProxy) (id : MaterialId) : Sample list =` (the live referencing lookup that feeds material removal). Cited seam present at the line; `materialsReferencingCategory` is the NEW analogue this cycle mints (not audited here).",
      "verdict": "CONFIRMED"
    },
    {
      "id": 21,
    ...
    },
    {
      ...
      "spec_location": "§A.1 Seeded catalogue and re-pointing",
      "evidence": "Read Domain/MaterialLibrary.fs:252 -> `category = Semiconductor` (the first seeded entry's — Silicon's — category assignment). The seeded-entries block sets `category = ...` on every entry from line 252 onward (through the vacuum entry at 316 to 340). Cited '252 onward' anchor present.",
      "verdict": "CONFIRMED"
    },
    {
      "id": 24,
      ...
      "spec_location": "§A.1 Seeded catalogue and re-pointing",
      "evidence": "Symbol-anchored: Grep TestWindows/MaterialEditorView.fs -> `let private categoryRow` at line 551 (unique). Cited symbol present (the 'offers only SelectableOnCreate' behaviour is the NEW change, not audited here).",
      "verdict": "CONFIRMED"
    },
    {
      "id": 25,
      ...
      "spec_location": "§A.1 Seeded catalogue and re-pointing",
      "evidence": "Symbol-anchored: Read TestWindows/TableAndElementRotationView.fs -> `materialCategories` (1910), and the `…Code`/`…Label`/`…OfCode` siblings resolve to `materialCategoryCode` (1915), `materialCategoryLabel` (1924), `materialCategoryOfCode` (1934). All present; cited `:1910` anchors the cluster (shorthand root is `materialCategory`, singular).",
      "verdict": "CONFIRMED"
    },
    {
      "id": 26,
      ...
      "evidence": "Class-anchored precedent reference: `module LibraryControls =` is defined once at Controls/LibraryControls.fs:20 (other grep hits are references/tests and the distinct `SampleLibraryControls`); resolves unambiguously. `<UICOMP:CategoryControls>` and `CategoryEditor` are NEW ids minted by this cycle (contract table), not audited here.",
      "verdict": "CONFIRMED"
    },
    {
      "id": 27,
      ...
      "spec_location": "§A.2 The mutating category proxy and the manager surface",
      "evidence": "[existing-artefact claim: 'the existing editor windows'] The named host project OpticalConstructor.TestWindows exists and contains editor windows including MaterialEditorWindow.fs and SampleEditorWindow.fs (Glob). The plural 'existing editor windows' placement claim resolves against those files.",
      "verdict": "CONFIRMED"
    }
  ]
}
