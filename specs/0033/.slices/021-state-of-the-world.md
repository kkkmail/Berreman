# State of the world — spec 0033, slice 021

# Where we are

Slice 021 gives the sample library a pure, Avalonia-free EDIT MODEL: where
slice 001 made sample stacks DATA (`SampleStructure`) and slice 020 hung
crystal orientation on each layer, this slice adds the message-DU editor over
that data — `OpticalConstructor.Domain.SampleStackEditor` — mirroring the
discipline of the Ui's `StackEditor.StackMsg` / `applyStackMsg` but at the
material-id level, so a later WIRE_UI slice can drive a sample editor panel by
dispatching messages into a windowless, fully unit-tested core. The existing
`OpticalSystem` editor (`StackEditor.groupLayers`) is untouched.

# What's working

- SampleStackEditor added to OpticalConstructor.Domain: SampleStackEditState (structure : SampleStructure, selection : Set<LayerPosition>) with ofStructure, LayerPosition (AtSingleLayer | AtCellLayer — a repetition adds no position), and the eleven-arm SampleStackMsg applied by applySampleStackMsg : msg -> state -> Result<state, SampleStackEditError>
- Selection arms: SelectLayer accumulates valid positions (invalid = no-op), SelectByMaterial replaces the selection with every layer referencing the MaterialId (singles + period cells), ClearSelection resets
- Edit arms map over exactly the selected layers: SetThicknessOfSelected / SetMaterialOfSelected / SetOrientationOfSelected; editing a period's cell slot edits every repetition; RemoveSelected drops a group whose cell empties; MoveSelectedUp/Down are pinned-frontier swaps per container with the selection remapped to follow
- MakeRepeatBlock folds a contiguous run of top-level single layers into ONE Repeated { cell; count }; SetRepeatCount resizes a group by whole periods; both reject count < 1 with the typed InvalidRepeatCount (the Ui validateRepeatCount rule restated in Domain), plus SelectionNotFoldable / NotARepeatGroup for the shape rejections
- +30 windowless tests in SampleStackEditorTests covering every message arm and both acceptance criteria: MakeRepeatBlock over 2 selected layers with count K expands to 2*K films, and SelectByMaterial → SetThicknessOfSelected updates exactly the matching layers (constructor tests 377 → 407)
...
`specs/0033/.artifacts/021-*.log` (all five re-captured fresh in attempt 02).

- `build` — solution builds Release/x64, exit 0, 0 errors; the 91 warnings are
  the pre-existing MSB3277/FS1125 noise, none from the new files.
...
- **The edit model is Domain data + one pure update, not a Ui concern.**
  `applySampleStackMsg : SampleStackMsg -> SampleStackEditState ->
  Result<SampleStackEditState, SampleStackEditError>` is the single seam a
  future WIRE_UI slice binds to; every operation is an immutable
  copy-and-update, so the state is separable from any window (the repo's
  "state is separable from the UI" mandate) and every arm is provable by a
  plain unit test.
- **A period cell has ONE editable identity.** `AtCellLayer (group, cell)`
  addresses the cell slot, not a repetition — so "editing a period's cell
  edits every repetition" falls out of the data model rather than being
  enforced by code, and `SetRepeatCount` adds/removes whole periods without
  invalidating any selection.
- **Uniform Result shape with typed, reason-carrying errors.** Selection/edit
  arms always return `Ok`; only the two repeat arms reject
  (`InvalidRepeatCount | SelectionNotFoldable | NotARepeatGroup`, each
  `of reason : string`). The `count >= 1` rule is the SAME rule
  `Validation.validateRepeatCount` enforces at the Ui boundary — restated in
  Domain (Domain cannot reference Ui) and cross-checked by a test that runs
  both.
- **Moves return a full old→new permutation** (movers and displaced
  neighbours), so the selection follows the layers and `AtCellLayer` group
  indices survive top-level moves; cell selections reorder within their own
  group's cell, top-level selections move within `films` with a `Repeated`
  group travelling as one unit.

# Deferred

- No UI surface dispatches these messages yet — wiring the sample editor
  panel to `applySampleStackMsg` is a later WIRE_UI slice.
- Selection scope is the films stack only: the message set has no
  ...
  vocabulary).
- Nested repeat blocks (a `Repeated` inside a cell) are not representable in
  `SampleStructure` and are correspondingly rejected by `MakeRepeatBlock`
  (`SelectionNotFoldable`); if the structure ever gains nesting, the fold and
  the position type both grow.

# Gotchas

- **`MakeRepeatBlock` clears the selection** (the folded positions no longer
  exist) and `RemoveSelected` clears it too — a caller that wants to keep
  editing the new group addresses it via its films index (`SetRepeatCount`)
  or re-selects.
- **`SelectLayer` accumulates but `SelectByMaterial` replaces** — chosen so
  multi-select can build a contiguous fold run while the acceptance's
  "SelectByMaterial then edit" reads as "the selection IS the matching set".
- Attempt 01 exited 0 without a state-of-the-world: it was killed mid
  `ui-smoke` advisory run after finishing the code, tests, and fsproj
  registrations. Attempt 02 kept the (already green) on-disk implementation,
  re-ran all five gates fresh, and completed the round outputs — recorded in
  the impl-log's Gotchas.
...
- `.manifest.state.json` (modified) and `.claude/` (untracked) are the
  arc-runner's / harness's own files — left alone, as in slices 001–020.
