  ...
  "claims": [
    {
      ...
      "spec_location": "§N.0 Problem statement (chunk 4/4, .spec-md lines 501-513)",
    ...
    },
    {
      ...
      "spec_location": "§N.0 Problem statement (chunk 4/4, .spec-md lines 501-513)",
      "evidence": "Read Berreman/OpticalConstructor/OpticalConstructor.Storage/RecentFiles.fs -> `[<Literal>] let maxRecent = 16` at :21-22 and pure `bump (path : string) (existing : string list) : string list` at :38 (case-insensitive dedup, push-to-front, List.truncate maxRecent — a bounded MRU whose invariant is IO-free, matching 'pure bump invariant').",
      "verdict": "CONFIRMED"
    },
    {
      "id": 4,
      ...
      "spec_location": "§N.0 Problem statement (chunk 4/4, .spec-md lines 501-513)",
      "evidence": "Trigger 'unchanged' + definite-article 'the per-control UiIds modules'. Class-anchored: Grep 'module UiIds' -> 21 production matches (11 under OpticalConstructor.Controls: SampleLibraryControls.fs:88, RotationControls.fs:62, Ribbon.fs:52, RendererControls.fs:112, RayPositionControls.fs:41, MaterialsControls.fs:101, LibraryControls.fs:77, LayerBandsControls.fs:43, ExperimentControls.fs:193, ElementPaletteControls.fs:42, CategoryControls.fs:71; 10 under OpticalConstructor.TestWindows view files). The multiple matches ARE the claim's content (the spec asserts plural per-control modules), so multi-match confirms rather than ambiguates. The id values the spec says stay unchanged exist today as [<Literal>]/string ids inside those modules.",
      "verdict": "CONFIRMED"
    }
  ]
}
