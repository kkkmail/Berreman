...
module SolverHandoffWindowTests =
    ...
    let ``the window mounts carrying its id, the summary list, the solver-comes-later message, and each status row`` () =
        HeadlessSession.run (fun () ->
            ...
            Assert.True(matchesId SolverHandoffView.UiIds.window window, "the window itself carries the SolverHandoffWindow id")
            Assert.True(isPresent window SolverHandoffView.UiIds.summaryList, "the summary list is missing")
            Assert.True(isPresent window SolverHandoffView.UiIds.solverMessage, "the solver-comes-later message is missing")
            Assert.Contains("separately", textOf window SolverHandoffView.UiIds.solverMessage)
            Assert.True(isPresent window (SolverHandoffView.UiIds.statusRow 1), "experiment 1 status row is missing")
            Assert.True(isPresent window (SolverHandoffView.UiIds.statusRow 2), "experiment 2 status row is missing")
            window.Close())
    ...
    let ``a valid experiment shows a ready status; a missing file and a range mismatch show their typed message`` () =
        HeadlessSession.run (fun () ->
            ...
            window.Show()
            Dispatcher.UIThread.RunJobs()
            Assert.Contains("Ready", textOf window (SolverHandoffView.UiIds.statusRow 1))
            Assert.Contains("Missing data file", textOf window (SolverHandoffView.UiIds.statusRow 2))
            Assert.Contains("Validation error", textOf window (SolverHandoffView.UiIds.statusRow 3))
            window.Close())
