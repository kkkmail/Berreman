...
module SampleProxyTests =
    ...
    let ``updateSample then tryGetSample returns the updated sample`` () =
        ...
        | other -> Assert.Fail(sprintf "expected the updated sample, got %A" other)
    ...
    let ``removeSample then searchSamples no longer finds the removed sample`` () =
        ...
        | other -> Assert.Fail(sprintf "expected exactly the langasite sample before removal, got %A" other)
